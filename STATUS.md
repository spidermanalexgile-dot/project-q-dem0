...
[2026-05-31T00:39:30Z] · DONE · prod 200; live: voice+analyst merged into one Project Q Assistant (mic in chat, commands auto-apply, questions answered, suggest+Apply multi-lever), regressions pass, 0 console errors · 14762ca
[2026-05-31T01:06:18Z] · assistant -> voice-only background widget: small mic orb moved bottom-left (out of the way), no text box/chat, listens + responds audibly + auto-applies suggested levers; ElevenLabs upgrade path in speech.ts (window.ProjectQ.setVoiceApiKey, falls back to browser voice); regressions pass; build clean; pushing · d168d20
[2026-05-31T01:08:38Z] · DONE · prod 200; live: voice-only background mic orb bottom-left (no text box, out of the way), brain works, ElevenLabs upgrade path (setVoiceApiKey) + browser fallback, regressions pass, 0 console errors · d09549e
[2026-05-31T01:33:58Z] · render Venice day capacity (Location shows 50k visitors/day, Crowd level shows headcount today = target × crowd%); Capacity ceiling lever now displays absolute visitors/day (200% → 100,000/day) instead of bare %, keeping % in the sub-line; slider+voice unchanged; build clean; pushing · 240ec2f
[2026-05-31T01:36:18Z] · DONE · prod 200; live: day capacity rendered (Location 50k visitors/day, Crowd level 100k today), Capacity ceiling shows 100,000/day not bare %, regressions pass, 0 console errors · 7278b8f
[2026-05-31T01:45:56Z] · rebased lever bounds to realistic Venice numbers (record ~110k): target_capacity max 350k→70k, max_fee_cap 1000→150, ceiling_pct 300%→220% (=110k at 50k target), base_fee 100→50; no more 400k+ unrealism; occupancy/demand regressions pass; build clean; pushing · 1a2430d
[2026-05-31T01:48:13Z] · DONE · prod 200; live: realistic Venice bounds (Capacity ceiling tops at 110k record not 150k+, target max 70k, cap €150, no 400k+ figures), regressions pass, 0 console errors · 4b39e9a
[2026-05-31T02:20:23Z] · ElevenLabs female-only voices: curated list of 7 named female voices (Rachel/Bella/Charlotte/Matilda/Grace/Lily/Elli), setVoiceApiKey + setVoice(name) + listVoices on window.ProjectQ; unknown/male name falls back to default female so never male; (Claude API has no TTS — clarified to user); build clean; verify pass; pushing · f5bc688
[2026-05-31T02:22:39Z] · DONE · prod 200; live: female-only ElevenLabs voices (7 named, listVoices/setVoice/setVoiceApiKey, unknown→female default so never male), Claude-API-has-no-TTS clarified, 0 errors · bd96ef2
[2026-05-31T02:45:03Z] · ElevenLabs SECURE server proxy (Option B): api/tts.ts edge fn reads ELEVENLABS_API_KEY server-side (never in browser), female-voice-id allowlist; speak() prefers /api/tts proxy (auto-probed) → client key → browser voice; window.ProjectQ.voiceStatus() reports active engine; key name absent from client bundle; dev falls back cleanly; build clean; pushing · f2b888b
[2026-05-31T02:47:27Z] · DONE · prod 200; live: secure /api/tts proxy deployed (GET {"ok":false} until key set), voice API + 7 female voices work, key name absent from bundle, 0 console errors; setup = add ELEVENLABS_API_KEY env var in Vercel + redeploy · a15b787
[2026-05-31T03:21:17Z] · DIAGNOSED voice setup: env var was named ElevenLabs_API_Key (case-mismatch, now matched tolerantly) + Edge→Node runtime fix → probe ok:true keyLen 51; but ElevenLabs returns 402 paid_plan_required (free plan cannot use library voices via API). Added session-disable fallback to browser voice on 402/5xx so never silent. Key+proxy correct; needs paid ElevenLabs plan · 63265c3
[2026-05-31T03:34:36Z] · DONE · staying free → browser female voice. Free ElevenLabs key blocked twice (402 paid_plan_required for library voices + 401 missing voices_read perm). Proxy now forwards real upstream status; client disables proxy on 401/402/429/5xx and falls to browser voice — verified live UI flips server-proxy→browser on first reply, never silent, 0 errors · da3dd1f
